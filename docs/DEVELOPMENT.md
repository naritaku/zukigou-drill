...
- バックオフ状態はインスタンスのメモリ上にのみ保持される。Cloud Run の
  再起動・スケールアウトでリセットされ、新インスタンスは制限中のキーを
  再試行する（429 を受けて再度バックオフに入る）
...
**永続化していない点に注意**: 状態は各インスタンスのメモリ上にのみ存在する。
Cloud Run の再起動やスケールアウトでリセットされ、新しいインスタンスは制限中の
キーを一度再試行する。恒久的に共有したい場合は Firestore/Redis 等の外部ストアが
必要だが、現状は未実装。
